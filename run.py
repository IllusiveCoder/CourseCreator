...
class MainWindow(QMainWindow):
    ...
    def createWordDocument(self):
        ...
        doc.addParagraph(f"Author: {self.authorInputText}").style = 'Subtitle'
        doc.addParagraph(f"Topics: {self.topicsInputText}").style = 'Subtitle'
        ...
        for line in lines:
            ...
            if line.startswith("#"):
                if line.startswith("##"):
                    # Unterüberschrift
                    if inParagraph:
                        doc.addParagraph().add_run(line[2:].strip().replace('\n', '')).bold = True
                        inParagraph = False
                    else:
                        doc.addParagraph().add_run(line[2:].strip().replace('\n', '')).bold = True
                else:
                    # Kapitelüberschrift
                    if inParagraph:
                        doc.addParagraph().add_run(line[1:].strip().replace('\n', '')).bold = True
                        inParagraph = False
                    else:
                        doc.addParagraph().add_run(line[1:].strip().replace('\n', '')).bold = True
            else:
                # Inhalt
                if not inParagraph:
                    doc.addParagraph().add_run(line.strip())
                    inParagraph = True
                else:
                    doc.addParagraph().add_run(line.strip())
